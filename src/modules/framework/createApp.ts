...
export const createApp = function (opt: Partial<CreateOptions> = {}) {
    ...
    const on = function (callbacks: AppLifecycleCallbacks): number {
        ...
        Object.keys(callbacks).forEach(type => {
            const callback = callbacks[type as keyof AppLifecycleCallbacks]
            if(callback) lifecycleCallbacks[type as keyof CallbackStore]?.push({ id, callback: callback })
        })
        return id
    }
    ...
    const run = function (): void {
        ...
        // sim房间不需要用自定义缓存
        if (_memoryCacher && !Game.rooms['sim']) _memoryCacher(_run)
        else _run()
    }
    ...
    const execLifecycleCallback = function (lifecycleType: keyof AppLifecycleCallbacks) {
        for (const { callback } of lifecycleCallbacks[lifecycleType as keyof CallbackStore] ?? []) {
